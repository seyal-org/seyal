...
//!
//! Leaf metadata alone leaves a pathname TOCTOU gap; after `connect` the
//! client must still prove the peer effective UID matches ours
...
pub fn verify_same_user_peer(socket: RawFd) -> io::Result<()> {
    ...
    if peer_uid != own_uid {
        return Err(io::Error::new(
            ...
            "peer effective UID does not match Agent client effective UID",
        ));
    }
    Ok(())
}
