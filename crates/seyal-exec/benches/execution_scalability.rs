...
mod macos {
    ...
    fn sample_contract(gate: &str, population: usize) -> (f64, u64) {
        ...
        match gate {
            "startup" => {
                ...
                assert!(
                    execution.terminal().rows() > 0 && execution.terminal().cursor().col > 0,
                    "startup did not reach a usable TerminalState after ready bytes"
                );
            ...
            "teardown_recovery" => {
                ...
                loop {
                    if ps_metrics(child).is_err() {
                        break;
                    }
                    if Instant::now() >= reap_deadline {
                        panic!("teardown_recovery did not reap child {child}");
                    }
                    std::thread::sleep(Duration::from_millis(10));
                }
                let _ = ps_metrics(std::process::id());
                (started.elapsed().as_secs_f64() * 1_000.0, 0)
            }
            "idle_cpu"
            ...
            | "resource_scaling_threads" => {
                ...
                let process = ps_metrics(std::process::id()).expect("contract process metrics");
                let child_rss: u64 = executions
                    .iter()
                ...
                println!(
                    "[seyal scalability] m002_topology process_rss_kib={} child_rss_kib={} threads={} fds={} population={} evidence=execution-only-headless-not-full-app",
                    process.rss_kib, child_rss, process.threads, fds, population
                );
                terminate_all(&mut executions);
                let value = match gate {
                    "idle_cpu" => process.cpu_percent,
                    "resource_scaling_rss" => (process.rss_kib as f64) * 1024.0,
                    "resource_scaling_fds" => fds,
                    "resource_scaling_threads" => process.threads as f64,
                    _ => unreachable!(),
                };
                (value, child_rss)
            }
    ...
    fn run_contract_cohort() {
        ...
        let mut last_child_rss = 0_u64;
        let mut last_process_rss = 0_u64;
        for _ in 0..samples {
            let (value, child_rss) = sample_contract(&gate, population);
            retained.push(value);
            last_child_rss = child_rss;
            if let Ok(process) = ps_metrics(std::process::id()) {
                last_process_rss = process.rss_kib;
            }
        }
        ...
        println!(
            "[seyal scalability] m002_contract gate={gate} cohort={cohort} warmups={warmups} samples={samples} population={population} process_rss_kib={last_process_rss} child_rss_kib={last_child_rss} out={out} performance_claim=false"
        );
    }
    ...
    fn ps_metrics(pid: u32) -> io::Result<ProcessMetrics> {
        let output = Command::new("ps")
            ...
            .output()?;
        // Dead/reaped PIDs make `ps` exit non-zero with empty stdout. Treat that
        // as Err so teardown_recovery's reap loop can progress instead of
        // panicking on fields[0] (harness bug; does not change gate ceilings).
