...
17a. [`ADR-017-NATIVE-WINDOW-TAB-LIFECYCLE.md`](ADR-017-NATIVE-WINDOW-TAB-LIFECYCLE.md) — **Proposed** (#1000): M003 native window/tab containment, Rust-owned `WindowId`/`TabId` identity and ordering, presentation tiers for inactive-but-live executions, presentation-close-is-not-terminate semantics, bounded quit, stale/concurrent action fencing and the M003/M004 durability boundary.
