...
17b. [`ADR-018-LOCAL-RESOURCE-ADDRESSING-NAVIGATION.md`](ADR-018-LOCAL-RESOURCE-ADDRESSING-NAVIGATION.md) — **Proposed** (#1004): typed local `ResourceAddress` for Workspace/Tab/Pane/Session-Execution navigation targets, fail-closed resolution and rejection taxonomy, reveal-and-focus navigation semantics, one application-scoped bounded focus history, placement-independent cross-window navigation, and the rule that a display label is never identity.
