...
17b. [`ADR-018-NATIVE-WINDOW-TAB-LIFECYCLE.md`](ADR-018-NATIVE-WINDOW-TAB-LIFECYCLE.md) — **Proposed** (#1000): M003 native window/tab containment, Rust-owned `WindowId`/`TabId` identity and ordering, presentation tiers for inactive-but-live executions, presentation-close-is-not-terminate semantics, bounded quit, stale/concurrent action fencing and the M003/M004 durability boundary.
17c. [`ADR-017-EXECUTION-PROVISIONING-AND-DISPOSITION.md`](ADR-017-EXECUTION-PROVISIONING-AND-DISPOSITION.md) — **Proposed** (#994; siblings #1000→ADR-018, #1004→ADR-019): how a new Tab or split Pane obtains one distinct Runtime-owned `TerminalExecution` and how it is disposed of. Rust product authority owns provisioning intent; the Runtime registry remains the single creation authority and launch-policy owner; `TabId`/`PaneId` never cross the local protocol; creation and explicit Controller-fenced termination are separate capability-gated operations.
17e. [`ADR-020-STARTUP-SHELL-ENV-CWD-LAUNCH-POLICY.md`](ADR-020-STARTUP-SHELL-ENV-CWD-LAUNCH-POLICY.md) — **Proposed** (#1003; parent #676): typed cold-path `EffectiveLaunchPolicy` for interactive shell program/argv (login default), startup CWD, bounded environment allowlist, and `TERM`/`COLORTERM` ownership consumed by the #994 / proposed ADR-017 provisioning seam. Neighbor ADR-017 is Proposed on PR #1056 and is not rewritten here. Provisional siblings: #994→ADR-017 (PR #1056), #1000→ADR-018 (merged #1055), #1004→ADR-019 (merged #1057).
...
- ADR-017 (**Proposed**, #994) owns the pane/tab execution provisioning and disposition seam: portable Rust product authority owns provisioning intent, the Runtime registry remains the single execution-creation authority and launch-policy owner, `TabId`/`PaneId` never cross the local protocol or become PTY/VT owners, provisioning and termination are separate capability-gated correlated operations, and closing presentation detaches rather than terminates. It does not move PTY/VT ownership, change Flow/Raw/TUI behavior, authorize presentation/layout persistence, raise SPEC-004 connection/attachment maxima, or decide config-declared launch profiles (#676) and trusted CWD signalling (#686). Observable behavior is amended into SPEC-004 §18, SPEC-003 §4.1/§5.2 and SPEC-009 §8.2.1.
- ADR-018 (**Proposed**) owns the M003 headed window/tab lifecycle boundary: `Workspace -> Window -> Tab -> PaneTree -> Pane` containment, Rust-owned `WindowId`/`TabId`/`PaneId` identity and ordering with AppKit as disposable realization, disallowed macOS native window tabbing, the rule that closing a Pane/Tab/Window or quitting never terminates a live `TerminalExecution`, presentation tiers for inactive-but-live executions, bounded Rust-owned quit, generation/identity action fencing, and the M003-versus-M004 durability split. It does not own the pane/tab execution-provisioning seam (#994), split ratios (#928), multiple live Metal leaves (#936) or durable layout persistence.
