...
- ADR-016 owns the independent OSS Agent Backend boundary: portable WorkScope identity, one per-user agent-domain daemon, Agent Backend/domain as the AgentRun/agent-originated Action writer, terminal Runtime as TerminalExecution/PTY/VT authority, secure client protocol, and truthful enforcement/request-assembly boundaries. It does not authorize M006 workflow orchestration or move terminal hot-path authority into the agent service.
