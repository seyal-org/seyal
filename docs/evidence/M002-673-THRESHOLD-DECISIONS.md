...
| `high_output_responsiveness` | 8 | 16 | 33 | ms | less_equal | Input admission to the first committed client `DisplayCache` generation containing that sample's unique marker, while a ≥2 s flood is live (post-cohort `assert_responder_flood_live`: at least five further flood generations with no client input, because `SustainedResponder` never emits `DONE`). Collector workload `SustainedResponder`: flood confined below row 1 (DECSTBM); responder writes each input line to row 1 with echo off so the marker cannot scroll out of a 24-row viewport (plain `Sustained` flood wraps ~52 rows/line and times out the correlated wait). One-to-two frame budget. |
